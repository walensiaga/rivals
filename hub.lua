...
local libraryCode = local UILibrary = {}
--// Modules
...
function UILibrary.new(gameName, userId, rank)
    ...
    --// Customize the GUI
    window.Watermark.Text = ("reduxhub | %s | %s"):format(userId, gameName)
...
local UILib = loadstring(libraryCode)
if not UILib then
    warn("Failed to compile library code")
    return
end
